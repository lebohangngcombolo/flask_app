...
# IMPORTANT: Ensure this is correctly picking up from .env or provides a full default
app.config['GOOGLE_REDIRECT_URI'] = os.getenv('GOOGLE_REDIRECT_URI', 'https://YOUR_CURRENT_NGROK_URL.ngrok-free.app/api/auth/google/callback')
# Fallback to a localhost URI for development by default, if ngrok isn't used or configured.
# If you are using ngrok, ensure your .env has the correct ngrok URL for GOOGLE_REDIRECT_URI.
app.config['FRONTEND_URL'] = os.getenv('FRONTEND_URL', 'http://localhost:5173')

# Apply ProxyFix to trust the headers from ngrok
# This tells Flask to recognize that it's behind a proxy that handles SSL/TLS
app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_host=1, x_proto=1, x_prefix=1)

print(f"Flask using GOOGLE_REDIRECT_URI: {app.config['GOOGLE_REDIRECT_URI']}")
...
allowed_origins = [
    ...
    app.config['FRONTEND_URL'] # Explicitly add frontend URL from config
]

# Add the base of the Google Redirect URI if it's an ngrok URL
if app.config['GOOGLE_REDIRECT_URI'] and "ngrok-free.app" in app.config['GOOGLE_REDIRECT_URI']:
    # Extract just the base URL (e.g., https://abc.ngrok-free.app) for CORS
    ngrok_base_url = app.config['GOOGLE_REDIRECT_URI'].split('/api/auth/google/callback')[0]
    if ngrok_base_url not in allowed_origins:
        allowed_origins.append(ngrok_base_url)

# Add production domain if applicable
production_domain = os.getenv('PRODUCTION_FRONTEND_URL') # You might want to add this to your .env
...
def role_required(allowed_roles):
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                token = token.split(' ')[1]  # Remove 'Bearer ' prefix
                # Corrected: Use app.config['JWT_SECRET_KEY'] consistently
...
def google_login():
    ...
    try:
        # Add this line here to print what Flask is using
        print(f"Flask using GOOGLE_REDIRECT_URI: {app.config['GOOGLE_REDIRECT_URI']}")
        ...
        flow = Flow.from_client_config(
            ...
            # Update scopes to use fully-qualified URLs
            scopes=['openid', 'https://www.googleapis.com/auth/userinfo.email', 'https://www.googleapis.com/auth/userinfo.profile']
        )
        
        # Explicitly set the redirect_uri on the flow object after creation
        ...
        print(f"Generated Google Authorization URL: {authorization_url}") 
...
def google_callback():
    ...
    try:
        ...
        # Explicitly set the redirect_uri on the flow object for the callback as well
        flow.redirect_uri = app.config['GOOGLE_REDIRECT_URI']
        ...
        if not user:
            ...
            user = User(
                email=email,
                full_name=full_name,
                phone='',  # You might want to handle this differently
                role='member',
        ...
        frontend_url = app.config['FRONTEND_URL'] # Use the configured FRONTEND_URL
        return redirect(f"{frontend_url}/auth/callback?token={access_token}")
        
    except Exception as e:
        print(f"Google authentication error: {str(e)}")
        frontend_url = app.config['FRONTEND_URL'] # Use the configured FRONTEND_URL
        return redirect(f"{frontend_url}/auth/error?message={quote(str(e))}")
