...
def upload_to_cloudinary(file, folder="kyc_docs", public_id=None):
    """
    Upload a file to Cloudinary and return the secure URL
    
    Args:
        file: FileStorage object from Flask request
        folder: Cloudinary folder name (default: kyc_docs)
        public_id: Optional custom public_id for the file
    ...
    try:
        ...
        if not public_id:
            ...
            public_id = f"{folder}/{timestamp}_{secure_filename(file.filename)}"
...
def update_kyc(current_user):
    ...
    if request.files:
        ...
        def save_file_to_cloudinary(field_name):
            file = files.get(field_name)
            if file and file.filename:
                try:
                    # Generate a unique public_id for the file
                    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                    public_id = f"kyc_docs/{current_user.id}/{field_name}_{timestamp}"
                    
                    # Upload to Cloudinary
                    cloudinary_url = upload_to_cloudinary(file, folder="kyc_docs", public_id=public_id)
        ...
        if 'documents.idDocument' in files:
            kyc.id_document_path = save_file_to_cloudinary('documents.idDocument')
        if 'documents.proofOfAddress' in files:
            kyc.proof_of_address_path = save_file_to_cloudinary('documents.proofOfAddress')
        if 'documents.proofOfIncome' in files:
            kyc.proof_of_income_path = save_file_to_cloudinary('documents.proofOfIncome')
        if 'documents.bankStatement' in files:
            kyc.bank_statement_path = save_file_to_cloudinary('documents.bankStatement')
...
def submit_kyc(current_user):
    ...
    def save_file_to_cloudinary(field):
        file = files.get(field)
        if file:
            try:
                # Generate a unique public_id for the file
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                public_id = f"kyc_docs/{current_user.id}/{field}_{timestamp}"
                ...
                print(f"Error uploading {field} to Cloudinary: {e}")
                raise e
        return None

    # Update document paths
    kyc.id_document_path = save_file_to_cloudinary('documents.idDocument') or kyc.id_document_path
    kyc.proof_of_address_path = save_file_to_cloudinary('documents.proofOfAddress') or kyc.proof_of_address_path
    kyc.proof_of_income_path = save_file_to_cloudinary('documents.proofOfIncome') or kyc.proof_of_income_path
    kyc.bank_statement_path = save_file_to_cloudinary('documents.bankStatement') or kyc.bank_statement_path
...
def download_kyc_document(current_user, filename):
    ...
    try:
        ...
        # If filename is actually a URL, redirect to it
