...
<<<<<<< HEAD
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // Make sure this URL matches your Flask backend's port (e.g., 5000 or 5001)
      ...
      if (response.ok && data.auth_url) {
        ...
        toast.error(data.error || 'Failed to initiate Google sign-in');
      }
    } catch (error) {
      console.error('Error initiating Google sign-in:', error);
      toast.error('Could not connect to Google sign-in service.');
    } finally {
      setIsLoading(false);
    }
  };
