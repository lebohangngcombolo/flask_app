<<<<<<< HEAD
    ...
    import { useNavigate, Link } from 'react-router-dom'; // Corrected import for useNavigate and Link
    import { login } from '../utils/auth'; // Corrected import for login utility
    ...
    const Login: React.FC = () => {
      ...
      const handleGoogleSignIn = async () => {
        setLoading(true); // Optional: Set loading state while redirecting
        try {
          // Make sure this URL matches your Flask backend's port (e.g., 5000)
          ...
          if (response.ok && data.auth_url) {
            ...
            toast.error(data.error || 'Failed to initiate Google sign-in'); // Using toast here
          }
        } catch (err) {
          console.error('Error initiating Google sign-in:', err);
          toast.error('Could not connect to Google sign-in service.'); // Using toast here
        } finally {
          setLoading(false);
        }
      };
            ...
            <div className="w-full max-w-[600px] bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl p-6">
              ...
              <form className="space-y-3" onSubmit={handleSubmit}>
                ...
                <div className="space-y-3">
                  <button
                    type="button"
                    onClick={handleGoogleSignIn} // <-- Added onClick handler here
