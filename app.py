...
class Beneficiary(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    account_number = db.Column(db.String(50))   # <--- This line must be here!
    bank_name = db.Column(db.String(100))       # <--- And this one!
...
def edit_beneficiary(beneficiary_id):
    ...
    if not beneficiary:
        return jsonify({"error": "Beneficiary not found"}), 404
    data = request.get_json()
    beneficiary.full_name = data.get('name', beneficiary.full_name)
...
@app.route('/api/beneficiary-profile', methods=['GET'])
@jwt_required()
def get_beneficiary_profile():
    ...
    return jsonify({
        ...
        "profilePicture": beneficiary.profile_picture,
        "dateOfBirth": beneficiary.date_of_birth.isoformat() if beneficiary.date_of_birth else None,
        "phone": beneficiary.phone,
        "email": beneficiary.email,
        "address": beneficiary.address
    })

@app.route('/api/beneficiary-profile', methods=['PUT'])
@jwt_required()
def update_beneficiary_profile():
    ...
    if not beneficiary:
        return jsonify({"error": "Beneficiary profile not found"}), 404
    beneficiary.full_name = data.get("fullName", beneficiary.full_name)
    beneficiary.id_number = data.get("idNumber", beneficiary.id_number)
    beneficiary.profile_picture = data.get("profilePicture", beneficiary.profile_picture)
    beneficiary.date_of_birth = data.get("dateOfBirth", beneficiary.date_of_birth)
    beneficiary.phone = data.get("phone", beneficiary.phone)
    beneficiary.email = data.get("email", beneficiary.email)
    beneficiary.address = data.get("address", beneficiary.address)
    db.session.commit()
    return jsonify({"message": "Profile updated successfully"})

@app.route('/api/beneficiary-profile', methods=['POST'])
@jwt_required()
def create_beneficiary_profile():
    user_id = get_jwt_identity()
    data = request.get_json()
    # Check if profile already exists
    if Beneficiary.query.filter_by(user_id=user_id).first():
